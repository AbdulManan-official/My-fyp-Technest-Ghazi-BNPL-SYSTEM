...
export default function ReportsScreen() {
  ...
  const monthsForPicker = Array.from({ length: 12 }, (_, i) => ({ label: new Date(0, i).toLocaleString('default', { month: 'long' }), value: i + 1 }));

  const predefinedDateRanges = getPredefinedDateRanges();

  const getReportDateRange = () => {
    if (selectedPeriodKey === 'customMonth') {
      const startDate = new Date(pickerYear, pickerMonth - 1, 1, 0, 0, 0);
      const endDate = new Date(pickerYear, pickerMonth, 0, 23, 59, 59);
      const monthLabel = monthsForPicker.find(m => m.value === pickerMonth)?.label || `Month ${pickerMonth}`;
  ...
  const processOrderDataForReport = (querySnapshot, reportPeriodConfig) => {
    let totalSalesDelivered = 0, pendingOrdersAmount = 0, activeOrdersAmount = 0,
        bnplOrdersAmountTotal = 0, codOrdersAmountTotal = 0, totalOrdersCount = 0,
        installmentPlanOrdersCount = 0, fixedDurationPlanOrdersCount = 0,
        overallTotalSalesInPeriod = 0;
    const statusCounts = {};
    ...
    return { totalSalesDelivered, pendingOrdersAmount, activeOrdersAmount, bnplOrdersAmountTotal, codOrdersAmountTotal, totalOrdersCount, statusCounts, installmentPlanOrdersCount, fixedDurationPlanOrdersCount, bnplUsersPaymentStatus, detailedOrdersForPdf, installmentsPaidThisPeriod, overallTotalSalesInPeriod };
  };
  ...
  const generatePdfForSummary = async () => {
    ...
    body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; margin: 20px; font-size: 10px; color: #333; }
    h1 { color: ${ACCENT_COLOR}; text-align: center; font-size: 20px; margin-bottom: 8px; font-weight: 600;}
    .report-subtitle { text-align: center; font-size: 11px; color: #555; margin-bottom: 15px;}
    h2 { font-size: 15px; color: ${PRIMARY_TEXT_COLOR}; border-bottom: 1.5px solid ${ACCENT_COLOR}; padding-bottom: 4px; margin-top: 20px; font-weight: 500;}
    .section { margin-bottom: 18px; padding: 12px; border: 1px solid #E0E0E0; border-radius: 6px; background-color: #FCFCFC;}
    .metric { display: flex; justify-content: space-between; padding: 5px 0; border-bottom: 1px dotted #EAEAEA;}
    .metric:last-child { border-bottom: none; }
    .metric-label { font-weight: 500; color: ${SECONDARY_TEXT_COLOR}; }
    .metric-value { font-weight: 600; color: ${PRIMARY_TEXT_COLOR}; }
    table { width: 100%; border-collapse: collapse; margin-top: 12px; font-size: 9px;}
    th, td { border: 1px solid #D0D0D0; padding: 5px; text-align: left; word-break: break-word; }
    th { background-color: #F0F0F0; font-weight: 600; }
    .user-details-cell { font-size:9px; }
    .user-details-cell b { color: #333; }
    .user-details-cell small { color: #555; display:block; margin-top:1px; }
    .address-cell { max-width: 150px; font-size: 8px !important; }
    </style></head><body>`;
              ...
              <View style={styles.pickerGroup}>
                ...
                <View style={styles.pickerInputContainer}>
                  <Picker
                    ...
                    style={styles.pickerComponent}
                    onValueChange={(itemValue) => {setPickerMonth(itemValue); setReportData(null);}}
                    itemStyle={styles.pickerItem} mode="dropdown"
                  >
...
const styles = StyleSheet.create({
  ...
  periodSegmentContainer: {
    ...
    borderColor: ACCENT_COLOR, overflow: 'hidden', marginBottom: 10, // Reduced margin
  },
  segmentButton: {
  ...
  segmentButtonFirst: { /* For potential future use if border radius is needed on individual segments */ },
  ...
  selectedPeriodLabelDisplay: {
    ...
    marginTop: 10, marginBottom: 20, // Adjusted spacing
  },
  ...
  pickerInputContainer: {
    ...
    borderColor: BORDER_COLOR, height: Platform.OS === 'ios' ? undefined : 55, // Explicit height for Android
    justifyContent: Platform.OS === 'ios' ? undefined :'center', overflow: 'hidden',
  },
  pickerComponent: { height: Platform.OS === 'ios' ? 180 : 55, width: '100%', color: PRIMARY_TEXT_COLOR }, // Added color
  pickerItem: { height: Platform.OS === 'ios' ? 180 : undefined, color: PRIMARY_TEXT_COLOR, fontSize: Platform.OS === 'ios' ? 20 : 16 }, // Matched font size
  ...
  actionButtonPdf: {
    backgroundColor: SUCCESS_COLOR, paddingVertical: 16, borderRadius: 12, // Green button
  ...
  userDetailLabel: { fontSize: 14, color: SECONDARY_TEXT_COLOR, fontWeight: '600', marginTop: 5, }, // Bolder label
